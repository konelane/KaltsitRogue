...
class GameNode:
    ...
    def isRqmtExist(self):
        ...
        return str(current_position) in self.node_json['rqmt_node_list']
        ...
        if self.isRqmtExist():
            ...
            if check_type == 'dice':
                ...
                if str(current_position) in list(self.node_json['dice_result'].keys()):
                    return int(self.node_json['dice_result'][str(current_position)]) >= int(check_dict[0])
                else:
                    return False
...
class DiceNode(GameNode):
    ...
    def inputDice(self, save_dict:GameSave):
        ...
        current_position = self.node_json['position_save']
        if self.isRqmtExist(): 
            out_list = self.node_json['dice_position'][str(current_position)]
            print(out_list)
            if out_list[2] == 'node_name':
                if self.rqmtCheck('dice', save_dict):
                    node_name_out = self.node_json['next_node_id']
    ...
    def cheatDiceWithInput(self, save_dict:GameSave, point_cheat):
        ...
        current_position = self.node_json['position_save']
        self.node_json['dice_result'][str(current_position)] = int(point_cheat) # 规则比较简单 可修改
...
class ChoiceNode(GameNode):
    ...
    def nextMove(self, save_dict:GameSave):
        '''读取下一步行动'''
        current_position = self.node_json['position_save'] 
        if current_position in self.node_json['choice_position_list']:
            return 'choice'
        else:
            return self.rqmtResultMove(save_dict)
